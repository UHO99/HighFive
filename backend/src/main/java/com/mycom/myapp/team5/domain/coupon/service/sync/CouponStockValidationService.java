...
/**
 * S013 - CLOSE된 쿠폰의 재고<->발급 이력 정합성을 주기적으로 재검증한다. S012와 달리 이미 동기화된 쿠폰도 매번 다시 검증한다 - "한 번 맞았으니 계속 맞다"고 가정하지 않는다. 단, 한 번 "드레인 완료 + 기록값=실측값"이 확인된 쿠폰은 coupon_issue가 더 이상 늘어날 일이 없으므로 consistencyConfirmedAt을 채워 재검증 대상에서 영구히 빼고, 그 시점에 Redis Stream도 함께 정리한다. 이렇게 하지 않으면 CLOSE 누적 건수만큼 매 사이클 부하가 무한정 커진다.
 */
@Slf4j
@Component
...
public class CouponStockValidationService {
	...
	// TODO: 발표 영상 촬영용으로 15초로 단축(원래 60_000) - 촬영 끝나면 60_000으로 되돌릴 것
	...
	private void verify(Coupon coupon, long actualIssuedCount, List<CouponMismatchReport> mismatches) {
		...
		CouponMismatchReport report = new CouponMismatchReport(coupon.getId(), coupon.getIssuedQuantity(), actualIssuedCount, pendingCount, coupon.getTotalQuantity());
	...
	private Map<Long, Long> countIssued(List<Coupon> coupons) {
		...
		return couponIssueRepository.countGroupedByCouponIds(couponIds).stream().collect(Collectors.toMap(CouponIssueRepository.CouponIssueCount::getCouponId, CouponIssueRepository.CouponIssueCount::getIssuedCount));
	}
