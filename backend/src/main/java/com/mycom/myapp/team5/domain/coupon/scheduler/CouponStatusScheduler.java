...
/**
 * 유스케이스 S010/S011 - 등록된 발급 기간(startAt/endAt) 에 맞춰 쿠폰 상태를 자동 전환 수동 API(open/close)와 동일한 CouponStatusService를 호출하므로 로직은 단일화
 */
@Slf4j
@Component
...
public class CouponStatusScheduler {
	...
	// TODO: 발표 영상 촬영용으로 5초로 단축(원래 60초) - 촬영 끝나면 60_000으로 되돌릴 것
	...
	public void replenishMissingStock() {
		...
		for (Coupon coupon : openCoupons) {
			if (Boolean.FALSE.equals(stringRedisTemplate.hasKey(CouponStockKeys.stockKey(coupon.getId())))) {
				...
				long issued = coupon.getIssuedQuantity() != null ? coupon.getIssuedQuantity() : couponIssueRepository.countByCouponIdAndStatus(coupon.getId(), CouponIssueStatus.ISSUED);
