...
/**
 * 대시보드 "쿠폰 발급 이력 · 선착순" 카드용 - Redis fairness-log의 실제 원자적 처리 순서(rank)에 DB의 상태/시각을 얹은 항목. rank는 Redis Lua 스크립트 안에서 재고 차감과 함께 매겨지므로 실제 처리 순서를 정확히 반영하고, DB 쪽(status/issuedAt)은 outcome이 SUCCESS여서 coupon_issue에 실제로 적재된 건에만 채워진다(SOLDOUT/DUPLICATE는 애초에 DB 행이 없다).
 */
public record CouponFairnessTimelineEntry(
		...
		/**
		 * 컨트롤러 도달 → Redis 게이트 진입까지 걸린 시간(ms) - validateIssueable() 등 게이트 진입 전 단계 소요. 시각 기록 추가 전에 쌓인 레거시 fairness-log 항목이면 null.
		 */
		Long controllerEnteredAtMs, // 추가 - 컨트롤러 도달 시각
		Long gateEnteredAtMs, // 추가 - Redis 게이트 진입 시각
		Long redisTimeMicros, // 추가 - Redis 실제 처리 시각
		Long gateWaitMs,
		/**
