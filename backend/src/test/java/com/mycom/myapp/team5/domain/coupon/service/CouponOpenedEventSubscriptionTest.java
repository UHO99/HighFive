...
/**
 * CouponOpenedEvent가 syncSubscriptions()의 2초 폴링을 기다리지 않고 즉시 구독을 트리거하는지 검증한다. 폴링 주기(2초)보다 훨씬 짧은 시간 안에 발급 이력이 저장되면, 이벤트가 실제로 동작한 것이다.
 */
@SpringBootTest
public class CouponOpenedEventSubscriptionTest {
	...
	public void openCoupon이_2초_폴링보다_먼저_구독을_등록한다() {
		Coupon coupon = Coupon.builder().name("이벤트-구독-테스트-쿠폰").totalQuantity(10).startAt(LocalDateTime.now().minusMinutes(1)).endAt(LocalDateTime.now().plusDays(1)).build();
		...
		couponIssueStreamProducer.requestIssue(couponId, userId, System.currentTimeMillis());

		// syncSubscriptions()의 폴링 주기(2초)보다 훨씬 짧게 잡는다 - 이 시간 안에 처리되면
		// 폴링이 아니라 이벤트가 구독을 걸었다는 뜻이다.
		await().atMost(Duration.ofMillis(800)).pollInterval(Duration.ofMillis(50)).untilAsserted(() -> assertThat(couponIssueRepository.countByCouponId(couponId)).isEqualTo(1L));
	}
